...
{
    if (code & 0x02)
        tab_key[0x71] = 1; // left shift
    if (code & 0x20)
        tab_key[0x71] = 1; // righr shift

    if (code & 0x04)
        tab_key[0x70] = 1; // left alt     0000 0100
    if (code & 0x40)
        tab_key[0x70] = 1; // right alt   0100 0000

    if (code & 0x01)
        tab_key[0x72] = 1; // right ctrl
    if (code & 0x10)
        tab_key[0x72] = 1; // left ctrl

    if (code & 0x08)
        tab_key[0x73] = 1; // win  l
    if (code & 0x80)
        tab_key[0x73] = 1; // win r
}
//----------------------------------------------------------
bool led_cl = true;
...
void key_on(uint8_t code) // клавиша нажата
{
    ...
    if (table_key_zx[code] == NC)
        return;                       // если на код нет нажатия клавиши ZX
